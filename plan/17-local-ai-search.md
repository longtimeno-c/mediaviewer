...
**Status: proposed 2026-09-24, post-v1. Not in PR 8. Milestone G, PRs 21–25.**
...
1. **`16-commands.md` lists "Face detect, AI cull, cloud albums" as out of scope** ("Rule 6; also
   ...
   is now planned as PR 25 below, local-only. Rule 6 holds because nothing leaves the machine;
...
- **Candidates:** SigLIP (Apache-2.0) and OpenAI CLIP ViT-B/32 (MIT), exported to ONNX.
  ViT-B/32-class at 224 px is the size/quality floor; go larger only if the eval set (PR 23)
...
- **Precision:** FP16 or INT8 (quantized) ONNX. Pick by measured recall loss in PR 21, not by
  assumption.
- Multilingual text queries are a *model* property (e.g. multilingual SigLIP variants), decided
  in PR 21 on the eval set. English-only is an acceptable first pack.
...
src/infer   IEmbedder { load(pack), embed_image(span<u8 rgb>, w, h), embed_text(string_view) }
            ...
            later coreml (Mac, Milestone F sibling)
```
...
- **The whole feature is a downloadable extra, installed from Settings** (owner call
  ...
  image/text model), *Faces* (PR 25), and one sub-pack per vendor provider. Remove deletes the
...
5. **Record** `pts` in the stream time base **and** as milliseconds, plus a flag for
   keyframe / grid-fill. Jump-to-moment must land where the frame is, so PR 22 stores the exact
...
- New command ids, added as **rows in the 16 table** when PR 23 lands, not before:
...
line ([14-abi.md](14-abi.md)). Sketch only; the header is written in PR 21:
...
continue after Milestone F to avoid renumbering PRs 9–20. **Windows first**; a Mac host is a
later sibling under D9 and is not scheduled here.

### PR 21 — Inference host and the AI pack
...
moments, kept out of git like the RAW corpus) recall@10 meets a target set from PR 21's numbers
...
### PR 25 — Faces (local people search)
...
sampled from photos and from the video keyframes PR 22 already extracts. Faces are clustered
...
**Verify:** on a labelled set, detection and clustering meet targets recorded from the PR 21
...
2. **Model** (SigLIP vs CLIP, size, multilingual) — decided by the PR 21 spike + eval set, with
