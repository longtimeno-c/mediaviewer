...
CreateSwapChainForComposition(  // or ForHwnd
  ...
  Format        = sdr ? DXGI_FORMAT_R8G8B8A8_UNORM_SRGB    // v1 default (D6)
...
- Handle `DXGI_ERROR_DEVICE_REMOVED` by rebuilding the device and reuploading from the CPU-side
  cache. It *will* happen — driver updates, hybrid-GPU switches, sleep/resume.
...
3. **Uploads are budgeted.** The render thread drains at most ~2 ms of texture uploads per frame
   from the ready-queue; the rest waits. A 100 MP TIFF landing must never cost you a dropped
   frame.
...
`IDXGIOutput::GetDesc` on `WM_DISPLAYCHANGE` and on window move, and re-pace.
...
- a **mip pyramid generated by a compute shader with a Kaiser/Mitchell kernel**, not
  `GenerateMips` (box filter, produces aliasing shimmer while panning),
